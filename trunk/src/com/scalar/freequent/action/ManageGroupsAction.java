...
public class ManageGroupsAction extends AbstractActionController {
    ...
    public void delete(Request request, Object command, Map<String, Object> data) throws ScalarActionException {
		...
		if (!GUID.isValid(id)) {
			throw ScalarActionException.create(MsgObjectUtil.getMsgObject(ActionResource.BASE_NAME, ActionResource.INVALID_PARAM_VALUE, groupName, id), null);
		}

        List<Item> items= itemDataService.search(params);
        if (items.size() > 0) {
            throw ScalarActionException.create(MsgObjectUtil.getMsgObject(ActionResource.BASE_NAME, ActionResource.UNABLE_TO_DELETE_ASSOCIATED_GROUP, ObjectType.GROUP, groupName), null);
        } else {
